<template>
  ...
  <the-section title="space">
    <d2-flex space="large">
      <d2-flex class="flex" space>
        <div class="flex__item" v-for="n in 3" :key="n" box="mean"/>
      </d2-flex>
      <d2-flex v-for="size in buttonSizeNames" :key="size" class="flex" :space="size" box="mean">
        <div class="flex__item" v-for="n in 3" :key="n"/>
      </d2-flex>
    </d2-flex>
  </the-section>
  <the-section v-for="_dir in dir" :key="_dir" :title="`dir:${_dir}`">
    <d2-flex space="large">
      <d2-flex class="flex" :dir="_dir" space>
        <div class="flex__item" v-for="n in 3" :key="n" box="mean"/>
      </d2-flex>
    </d2-flex>
  </the-section>
